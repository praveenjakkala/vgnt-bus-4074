...
import { Users, Bus, AlertTriangle, CreditCard, Search, Map, ExternalLink, Activity, DollarSign, LogOut } from 'lucide-react';
    ...
    const renderOverview = () => (
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    { label: 'Total Students', value: stats.totalStudents, icon: Users, color: 'text-vignan-blue dark:text-vignan-cyan', bg: 'bg-indigo-50 dark:bg-indigo-900/20' },
                    { label: 'Total Buses', value: stats.totalBuses, icon: Bus, color: 'text-vignan-cyan dark:text-vignan-cyan', bg: 'bg-cyan-50 dark:bg-cyan-900/20' },
                    { label: 'Pending Fees', value: stats.pendingFees, icon: DollarSign, color: 'text-red-500 dark:text-red-400', bg: 'bg-red-50 dark:bg-red-900/20', isCurrency: true },
                    { label: 'Active Alerts', value: stats.activeAlerts, icon: Activity, color: 'text-orange-500 dark:text-orange-400', bg: 'bg-orange-50 dark:bg-orange-900/20' },
                ].map((stat, i) => {
                    ...
                    return (
                        <motion.div key={i} custom={i} variants={cardVariants} initial="hidden" animate="visible"
                            ...
                            className="bg-white dark:bg-slate-900 rounded-xl p-5 shadow-sm border border-gray-100 dark:border-slate-800 relative overflow-hidden group">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden">
            <div className="p-4 border-b border-gray-100 dark:border-slate-800 flex items-center space-x-3">
                ...
                <input type="text" placeholder="Search by name or roll number..." value={searchTerm}
                    ...
                    className="w-full outline-none text-sm text-gray-700 dark:text-gray-100 bg-transparent placeholder-gray-400" />
            </div>
            {loadingStudents ? (
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        <thead className="bg-gray-50 dark:bg-slate-800/50 text-xs uppercase text-gray-500 dark:text-gray-400 font-bold">
                            <tr>
                        ...
                        <tbody className="divide-y divide-gray-50 dark:divide-slate-800">
                            {filteredStudents.map((s, i) => (
                                <motion.tr key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
                                    ...
                                    <td className="p-4 font-mono text-gray-500 dark:text-gray-400 text-xs">{s.id}</td>
                                    <td className="p-4">
                                        <span className={`px-2 py-1 rounded-full text-xs font-bold ${s.feeStatus === 'Paid' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400'}`}>
                                            {s.feeStatus}
                                        </span>
                                    </td>
                                    <td className="p-4 text-gray-600 dark:text-gray-300">₹{s.pending.toLocaleString()}</td>
                                    <td className="p-4 text-gray-500 dark:text-gray-400 text-xs">{s.route}</td>
                                </motion.tr>
                            ))}
                        </tbody>
                    </table>
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-slate-950 font-sans transition-colors duration-300">
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div className="flex items-center gap-4 border-l dark:border-slate-800 pl-6">
                            <div className="text-right">
                                <p className="text-xs text-gray-500 dark:text-gray-400 uppercase font-bold tracking-wider leading-none mb-1">Management Portal</p>
                                <p className="text-sm font-black text-vignan-blue dark:text-vignan-cyan leading-none">VGNT Bus 4074</p>
                            </div>
                            <motion.button
                                whileHover={{ scale: 1.1, rotate: 90 }}
                                whileTap={{ scale: 0.9 }}
                                onClick={() => navigate('/')}
                                className="p-2 bg-gray-50 dark:bg-slate-800 rounded-full text-gray-400 hover:text-red-500 transition-all border border-gray-100 dark:border-slate-800"
                            >
            ...
            <div className="max-w-7xl mx-auto p-4">
                {/* Tab Bar */}
                <div className="flex space-x-1 bg-white dark:bg-slate-900 p-1 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 mb-6 overflow-x-auto">
                    {tabs.map(tab => {
                        const Icon = tab.icon;
                        const isActive = activeTab === tab.id;
                        return (
                            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                                className={`flex items-center space-x-2 px-4 py-2.5 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${isActive ? 'bg-vignan-blue text-white shadow' : 'text-gray-500 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800'}`}>
                ...
                {activeTab === 'seats' && (
                    <div className="bg-white dark:bg-slate-900 rounded-xl p-6 shadow-sm border border-gray-100 dark:border-slate-800">
                        <h2 className="font-bold text-gray-800 dark:text-white mb-4 flex items-center"><Bus className="w-5 h-5 mr-2 text-vignan-blue" /> Bus AP39 UW 4074 — Seat Map</h2>
                        <div className="dark:brightness-90"><SeatLayout occupiedSeats={occupiedSeats} onSeatClick={() => { }} /></div>
                    </div>
                )}
            </div>
        </div>
